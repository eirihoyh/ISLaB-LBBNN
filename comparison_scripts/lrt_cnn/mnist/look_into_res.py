...
for i in range(n_nets):  # Use the list from mnist_lrt.py
    print(f"Testing net {i}")
    model = torch.load(f"comparison_scripts/lrt_cnn/mnist/network/net{i}_lr_0.005", weights_only=False)#,map_location=torch.device('cpu'))
    model.to(DEVICE)
...
df.to_csv('comparison_scripts/lrt_cnn/mnist/results/lrt_networks_mnist_lr_0.005_results.csv', index=False)
