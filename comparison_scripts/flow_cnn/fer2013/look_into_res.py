...
for i in range(n_nets):  # Use the list from fer2013_flow.py
    print(f"Testing net {i}")
    model = torch.load(f"comparison_scripts/flow_cnn/fer2013/network/net{i}", weights_only=False)#,map_location=torch.device('cpu'))
    model.to(DEVICE)
...
df.to_csv('comparison_scripts/flow_cnn/fer2013/results/flow_networks_fer2013_results.csv', index=False)
