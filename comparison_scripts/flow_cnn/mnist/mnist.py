...
for ni in range(n_nets):
    ...
    scheduler = MultiStepLR(optimizer, milestones=[int(0.6*tot_rounds), int(0.9*tot_rounds)], gamma=0.7) # int(0.3*tot_rounds), int(0.5*tot_rounds), int(0.7*tot_rounds), 
    ...
    # Results
    if save_res:
        torch.save(net, f"comparison_scripts/flow_cnn/mnist/network/net{ni}")
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'comparison_scripts/flow_cnn/mnist/results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mnist_leaky_relu_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'comparison_scripts/flow_cnn/mnist/results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mnist_leaky_relu_median.txt',m_median,delimiter = ',')