...
for i in range(n_nets):  # Use the list from cifar10_flow.py
    print(f"Testing net {i}")
    model = torch.load(f"comparison_scripts/flow_cnn/cifar10/network/net{i}", weights_only=False)#,map_location=torch.device('cpu'))
    model.to(DEVICE)
...
df.to_csv('comparison_scripts/flow_cnn/cifar10/results/flow_networks_cifar10_results.csv', index=False)
