...
# Get linear data
X_train_original = np.loadtxt("data/mice/X_train.txt", delimiter=",")
X_test_original = np.loadtxt("data/mice/X_test.txt", delimiter=",")
y_train_original = np.loadtxt("data/mice/Y_train.txt", delimiter=",")
y_test_original = np.loadtxt("data/mice/Y_test.txt", delimiter=",")
...
for ni in range(n_nets):
    ...
    for name, param in net.named_parameters():
        if f"lambdal" in name:
            alpha_lr = {'params': param, 'lr': lr*0.2}
    ...
    if save_res:
        torch.save(net, f"implementations/lrt/mice/network/net{ni}")
    all_nets[ni] = net 
    # Results
    metrics, metrics_median = pip_func.test_ensemble(all_nets[ni], test_dat, DEVICE, SAMPLES=100, CLASSES=n_classes, multiclass=multiclass, reg=(not class_problem)) # Test same data 10 times to get average 
    metrics_several_runs.append(metrics)
    metrics_median_several_runs.append(metrics_median)
    pf.run_path_graph(all_nets[ni], threshold=0.5, save_path=f"implementations/lrt/mice/path_graphs/prob/net{ni}_sigmoid", show=False)
    pf.run_path_graph_weight(net, save_path=f"implementations/lrt/mice/path_graphs/weight/net{ni}_sigmoid", show=False)
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'implementations/lrt/mice/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mice_sigmoid_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'implementations/lrt/mice/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mice_sigmoid_median.txt',m_median,delimiter = ',')