...
alphas_levels = [0.001,0.01,0.1,0.25,0.5,0.75,0.9,0.99,0.999]

all_nets = {}
metrics_several_runs = []
metrics_median_several_runs = []

for ni in range(len(alphas_levels)-1):

    lower_init_lambda = np.log(alphas_levels[ni]/(1-alphas_levels[ni]))
    upper_init_lambda = np.log(alphas_levels[ni+1]/(1-alphas_levels[ni+1]))
    ...
    net = BayesianNetwork(
        ...
        high_init_covariate_prob=False).to(DEVICE)
    ...
    if save_res:
        torch.save(net, f"implementations/lrt/abalone/diff_init_density/network/net_alphas_{alphas_levels[ni]}_{alphas_levels[ni+1]}")
    all_nets[ni] = net 
    # Results
    ...
    pf.run_path_graph(all_nets[ni], threshold=0.5, save_path=f"implementations/lrt/abalone/diff_init_density/path_graphs/prob/net_alphas_{alphas_levels[ni]}_{alphas_levels[ni+1]}", show=False)
    pf.run_path_graph_weight(net, save_path=f"implementations/lrt/abalone/diff_init_density/path_graphs/weight/net_alphas_{alphas_levels[ni]}_{alphas_levels[ni+1]}", show=False)
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'implementations/lrt/abalone/diff_init_density/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_abalone_sigmoid_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'implementations/lrt/abalone/diff_init_density/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_abalone_sigmoid_median.txt',m_median,delimiter = ',')