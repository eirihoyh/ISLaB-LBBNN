...
for ni in range(n_nets):
    ...
    # Results
    if save_res:
        torch.save(net, f"implementations/local_explain/mnist/network/net{ni}_dense_init")
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'implementations/local_explain/mnist/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mnist_relu_dense_init_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'implementations/local_explain/mnist/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mnist_relu_dense_init_median.txt',m_median,delimiter = ',')