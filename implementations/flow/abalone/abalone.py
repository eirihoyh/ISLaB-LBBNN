...
for ni in range(n_nets):
    ...
    if save_res:
        torch.save(net, f"implementations/flow/abalone/network/net{ni}")
    all_nets[ni] = net 
    # Results
    ...
    pf.run_path_graph(all_nets[ni], threshold=0.5, save_path=f"implementations/flow/abalone/path_graphs/prob/net{ni}_sigmoid", show=False)
    pf.run_path_graph_weight(net, save_path=f"implementations/flow/abalone/path_graphs/weight/net{ni}_sigmoid", show=False, flow=True) # NOTE: Forgot to run with flow=True
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'implementations/flow/abalone/results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_abalone_sigmoid_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'implementations/flow/abalone/results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_abalone_sigmoid_median.txt',m_median,delimiter = ',')