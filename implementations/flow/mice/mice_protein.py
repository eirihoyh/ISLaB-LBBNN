...
for ni in range(n_nets):
    ...
    if save_res:
        torch.save(net, f"implementations/flow/mice/network/net{ni}")
    all_nets[ni] = net 
    # Results
    ...
    pf.run_path_graph_weight(net, save_path=f"implementations/flow/mice/path_graphs/weight/net{ni}_sigmoid", show=False, flow=True)
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'implementations/flow/mice/results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mice_sigmoid_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'implementations/flow/mice/results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_mice_sigmoid_median.txt',m_median,delimiter = ',')