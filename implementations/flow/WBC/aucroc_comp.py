...
for i in range(n_nets):  # Use the list from wbc_flow.py
    print(f"Testing net {i}")
    model = torch.load(f"implementations/flow/WBC/network/net{i}", weights_only=False)#,map_location=torch.device('cpu'))
    model.to(DEVICE)
...
df.to_csv('implementations/flow/WBC/results/flow_networks_roc_auc_results.csv', index=False)
