...
# Get linear data
X_test_original = np.loadtxt(f"data/linear/0.0X_test.txt", delimiter=",")
y_test_original = np.loadtxt(f"data/linear/0.0Y_test.txt", delimiter=",")
...
path = "implementations/flow/linear_problem/"


# dep_levels = [0.0,0.1,0.5,0.9]
# for d in dep_levels:
#     test_res = []
#     X_test_original = np.loadtxt(f"data/linear/{d}X_test.txt", delimiter=",")
#     y_test_original = np.loadtxt(f"data/linear/{d}Y_test.txt", delimiter=",")
#     test_dat = torch.tensor(np.column_stack((X_test_original,y_test_original)),dtype = torch.float32)
#     for i in range(n_nets):
#         net = torch.load(path+f"network/net{i}_dep_level_{int(100*d)}", weights_only=False,map_location=torch.device('cpu'))
#         pf.save_metrics(net, path=path+f"results/net{i}_dep_level_{int(100*d)}")
...
#     np.save(path+f"results/ece_dep_level_{int(100*d)}", test_res)

dep_levels = [0,10,50,90]
for d in dep_levels:
    print(f"Dep level: {d}")
    test_res = np.load(path+f"results/ece_dep_level_{d}.npy", allow_pickle=True)
...
res = {}
for d in dep_levels:
    data = np.loadtxt(path+f"results/flow_class_skip_4_hidden_20_dim_200_epochs_0.01_lr_lin_func_dep_level_{d}_sigmoid_full.txt", delimiter=",")
    data_median = np.loadtxt(path+f"results/flow_class_skip_4_hidden_20_dim_200_epochs_0.01_lr_lin_func_dep_level_{d}_sigmoid_median.txt", delimiter=",")
    res[d] = {}
...
for d in dep_levels:
    ...
    for n in range(n_nets):
        median_info = np.load(path+f"results/net{n}_dep_level_{d}_median.npy", allow_pickle=True).item()
...
p = len(np.load(path+f"results/net{0}_dep_level_10_median.npy", allow_pickle=True).item()["include_inputs"][0])
res = {}
for d in dep_levels:
    res[d] = {}
    for i in range(p):
